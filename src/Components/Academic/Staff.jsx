// src/Academic/Teacher.jsx  (Staff list)
import React, { useEffect, useState } from 'react';
import { Link } from 'react-router-dom'; // ✅ web routing
...
const Staff = () => {
  ...
  if (loading) {
    // 🔵 SHIMMER: mirrors heading + grid + card (image left, text right)
  ...
  return (
    <div className='bg-blue-50 dark:bg-black/80'>
      ...
      <h3 className="text-3xl md:text-4xl font-bold dark:text-white/90 text-blue-900 text-center mb-4">Our Staffs</h3>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-8 md:gap-6 mb-8">
        {staff.map((s) => (
          <Link
            key={s.id}
            to={`/staff/${s.id}`} // keeps your existing route
          ...
          >
            <div className="flex items-center gap-4 sm:gap-4">
              ...
              <div className="mt-0">
                ...
                <p
                  className="text-lg sm:text-xl text-black/85 font-bold uppercase"
                  dangerouslySetInnerHTML={{ __html: String(s.name ?? '') }}
                />
                <p
                  className="font-semibold mt-2 text-gray-600"
                  dangerouslySetInnerHTML={{ __html: String(s.designation?.title ?? '') }}
                />
                <p className="font-semibold mt-2 text-gray-600">+88{s.phone}</p>
              </div>
            </div>
          </Link>
        ))}
